...
#Region " Declarations "
    ...
    Private cSecondaryOrderBy As String
#End Region
...
#Region " Properties "
    Public WriteOnly Property SecondaryOrderBy() As String
        Set(ByVal Value As String)
            cSecondaryOrderBy = Value
        End Set
    End Property
    ...
    Public WriteOnly Property FormatAsSubTable() As Boolean
        ...
        End Set
    End Property
    Public Property ChildTables() As ChildTablesClass
        Get
            Return cChildTables
        End Get
        Set(ByVal Value As ChildTablesClass)
            cChildTables = Value
        End Set
    End Property
...
#Region " Generate SQL "
    ...
    Public Sub GenerateSQL(ByRef Sql As String, ByRef ShowFilter As Boolean, ByVal SecurityWhereClause As String, ByVal OrderByType As DG.OrderByType, ByRef Request As HttpRequest, ByVal FilterOnOffState As String, ByVal FilterShowHideToggle As String, Optional ByVal OmitWhere As Boolean = False)
        ...
        If cSecondaryOrderBy <> Nothing Then
            If cNewOrderByField <> cSecondaryOrderBy Then
                Sql &= ", " & cSecondaryOrderBy
            End If
        End If
    ...
    Public Function GetLastFieldSorted() As String
        ...
        For i = 1 To cSortReferenceColl.Count
            If cSortReferenceColl(i).LastFieldSorted = "T" Then
                ...
                Result = cSortReferenceColl(i).SortExpression
            End If
        Next
...
#Region " Handle the internal filter "
        ...
        For i = 1 To cInternalFilter.Coll.Count
            If cInternalFilter.Coll(i).GetValue <> String.Empty Then
                ...
                Else
                    ...
                    ElseIf cInternalFilter.Coll(i).IsNonSpecific Then
                        SelectedValue = cInternalFilter.Coll(i).GetValue
                        'Coll.Add(cInternalFilter.Coll(i).Coll(SelectedValue).Sql)
                    End If
                End If
    ...
    Private Function GetDateCtlDateRange(ByVal DataFldName As String, ByVal Value As String) As Collection
        ...
        Dim FromMonthYear As String = Nothing
        Dim ToMonthYear As String = Nothing
        Dim Dates As Object
...
#Region " GetText et al "
    ...
    Private Function AffixMenuBand(ByVal EmbeddedMessage As String) As String
        ...
        ' ___ How this works
        ' If a filter exists, the menu displays the Show/Hide filter link and the Go link.
        ' Then, if the menu object exists, the menu displays the items in the menu collection.
        ' Whether or not any menu items appear, this methods adds a 30px row.
        ...
        If InternalFilterExists Or ExternalFilterExists Then
            sbMenu.Append("<td  align='center' width='" & Width & "%'  class=""DGMenuItem""><a href=""javascript:ApplyFilter()"">Go</a></td>")
        End If
    ...
    Private Sub HandleTemplateColumn(ByVal ColNum As Integer, ByRef dt As Data.DataTable, ByVal RowNum As Integer, ByRef sb As System.Text.StringBuilder)
        ...
        For j = 1 To TemplateCol.Count
            ...
            If TemplateCol.Coll(j).PermissionFldName = Nothing Then
            ...
            Else
                ...
                If dt.Rows(RowNum)(TemplateCol.Coll(j).PermissionFldName).ToText = "1" Then
                    PassPermissionFldTest = True
                End If
            End If

            ' ___ Rights test
            If TemplateCol.Coll(j).Right = Nothing Then
                PassRightTest = True
            Else
                If cRights.HasThisRight(TemplateCol.Coll(j).Right) Then
                    PassRightTest = True
                End If
            End If
            ...
            If IsVisible Then
                sb.Append("<a ")
                sb.Append("id=""" & TemplateCol.Coll(j).DataFldName & """ ")

                If TemplateCol.Coll(j).Parm2 = Nothing Then
                    sb.Append("onclick=""" & TemplateCol.Coll(j).OnclickMethod & "('" & dt.Rows(RowNum)(cKeyFieldName).ToJSParm & "')"">")
                ElseIf TemplateCol.Coll(j).Parm3 = Nothing Then
                    ...
                    sb.Append("onclick=""" & TemplateCol.Coll(j).OnclickMethod & "('" & dt.Rows(RowNum)(cKeyFieldName).ToJSParm & "', '" & dt.Rows(RowNum)(TemplateCol.Coll(j).Parm2).ToJSParm & "')"">")
                ElseIf TemplateCol.Coll(j).Parm4 = Nothing Then
                    ...
                    sb.Append("onclick=""" & TemplateCol.Coll(j).OnclickMethod & "('" & dt.Rows(RowNum)(cKeyFieldName).ToJSParm & "', '" & dt.Rows(RowNum)(TemplateCol.Coll(j).Parm2).ToJSParm & "', '" & dt.Rows(RowNum)(TemplateCol.Coll(j).Parm3).ToJSParm & "')"">")
                Else
                    sb.Append("onclick=""" & TemplateCol.Coll(j).OnclickMethod & "('" & dt.Rows(RowNum)(cKeyFieldName).ToJSParm & "', '" & dt.Rows(RowNum)(TemplateCol.Coll(j).Parm2).ToJSParm & "', '" & dt.Rows(RowNum)(TemplateCol.Coll(j).Parm3).ToJSParm & "', '" & dt.Rows(RowNum)(TemplateCol.Coll(j).Parm4).ToJSParm & "')"">")
                End If

                If TemplateCol.Coll(j).UseDefaultImage Then
                    sb.Append(cColumnColl(ColNum).DefaultImageColl(TemplateCol.Coll(j).DefaultImage))
                Else
                    sb.Append("<img src=""" & TemplateCol.Coll(j).ImagePath & """ ")
                    sb.Append(TemplateCol.Coll(j).ImageAttributes)
                End If

                sb.Append("title=""" & TemplateCol.Coll(j).Title & """> ")
                sb.Append("</a>")
            End If
        Next
        sb.Append("</td>")
...
#Region " Column classes "
    ...
    Public Class ChildTableSelectColumn
        ...
        Public ReadOnly Property ParmColl() As Collection
            ...
            End Get
        End Property
        Public ReadOnly Property EnableFldName() As String
            Get
    ...
    Public Class TemplateColumn
        ...
        'Default Public ReadOnly Property Coll(ByVal Idx As Integer) As Collection
        '    Get
        '        Return cColl(Idx)
        '    End Get
        'End Property
...
#Region " Filter "
    Public Class Filter
        ...
        Public Sub AddDropdown(ByVal ItemName As String, ByVal DataFldName As String)
            cColl.Add(New DropdownCtl(ItemName, True, False, False, DataFldName), ItemName)
        End Sub

        Public Sub AddExtendedDropdown(ByVal ItemName As String, ByVal DataFldName As String)
            cColl.Add(New DropdownCtl(ItemName, False, True, False, DataFldName), ItemName)
        End Sub

        Public Sub AddNonSpecificDropdown(ByVal ItemName As String, ByVal DataFldName As String)
            cColl.Add(New DropdownCtl(ItemName, False, False, True, DataFldName), ItemName)
        End Sub
        ...
        Public Class DropdownCtl
            ...
            Private cIsNonSpecific As Boolean
            ...
            Public ReadOnly Property IsNonSpecific() As Boolean
                Get
                    Return cIsNonSpecific
                End Get
            End Property
            ...
            Public Sub New(ByVal ItemName As String, ByVal IsStandard As Boolean, ByVal IsExtended As Boolean, ByVal IsNonSpecific As Boolean, ByVal DataFldName As String)
                MyBase.new(ItemName, DataFldName, False, True, False)
                cIsStandard = IsStandard
                cIsExtended = IsExtended
                cIsNonSpecific = IsNonSpecific
            End Sub
            ...
            Public Overrides Sub AddNonSpecificDropdownItem(ByVal Value As String, ByVal Text As String, Optional ByVal Selected As Boolean = False)
                If Selected Then
                    cSelectedValue = Value
                End If
                cColl.Add(New DropdownItem(Value, Text, Nothing), Value)
            End Sub
        ...
        Public Class FilterItem
            ...
            Public ReadOnly Property CtlName() As String
                Get
                    ...
                    If cIsTextBox Then
                        Name = "txt" & cDataFldName
                    ElseIf cIsDropdown Then
                        Name = "dd" & cDataFldName
                    End If
                    Return Name
                    'If cIsTextBox Then
            ...
            Public Overridable Sub AddNonSpecificDropdownItem(ByVal Value As String, ByVal Text As String, Optional ByVal Selected As Boolean = False)
            End Sub
...
#Region " ChildTables "
    Public Class ChildTablesClass
        ...
        Public Property ChildTableSelectColumn() As ChildTableSelectColumn
            Get
                Return cChildTableSelectColumn
            End Get
            Set(ByVal Value As ChildTableSelectColumn)
